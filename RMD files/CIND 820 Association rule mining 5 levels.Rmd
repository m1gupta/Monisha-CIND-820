---
title: "CIND 820 association rule mining with 5 levels"
author: "Monisha"
date: "2022-11-20"
...
```{r}
quantile(df_medicare_covid_clean$Total_Bene_Hosp, probs = c(0,0.1, 0.35, 0.65, 0.9,1))
```
...
df_medicare_covid_categorical$Total_Bene_Hosp <- cut(df_medicare_covid_clean$Total_Bene_Hosp, 
                   breaks=c(-Inf, 21, 153, 512, 3935.4, 1098471), 
...
rules_over_1 <- apriori(df_over_trans,
                 parameter = list(supp = 0.001, conf = 0.8), appearance = list(rhs=c("Total_Bene_Hosp=very low")))
...
rules_over_4 <- apriori(df_over_trans,
                 parameter = list(supp = 0.001, conf = 0.8), appearance = list(rhs=c("Total_Bene_Hosp=high")))

rules_over_5 <- apriori(df_over_trans,
                 parameter = list(supp = 0.001, conf = 0.8), appearance = list(rhs=c("Total_Bene_Hosp=very high")))
```
...
```{r}
inspect(rules_over_2)
```
```{r}
inspect(rules_over_3[1:10])
```
```{r}
inspect(rules_over_4)
```
```{r}
inspect(rules_over_5[1:10])
```

UnderSampling
```{r}
rules_under_1 <- apriori(df_under_trans,
                 parameter = list(supp = 0.001, conf = 0.85), appearance = list(rhs=c("Total_Bene_Hosp=very low")))
...
rules_under_4 <- apriori(df_under_trans,
                 parameter = list(supp = 0.001, conf = 0.85), appearance = list(rhs=c("Total_Bene_Hosp=high")))

rules_under_5 <- apriori(df_under_trans,
                 parameter = list(supp = 0.001, conf = 0.85), appearance = list(rhs=c("Total_Bene_Hosp=very high")))
```
```{r}
inspect(rules_under_1[1:10])
```
```{r}
inspect(rules_under_2)
```
```{r}
inspect(rules_under_3)
```
```{r}
```
```{r}
inspect(rules_under_4)
```
```{r}
inspect(rules_under_5[1:10])
```
